classdef circularGraph < handle
    ...
    methods
        function this = circularGraph(adjacencyMatrix,varargin)
            ...
            defaultLineMask = ones(size(adjacencyMatrix, 1), 1); % Default to all ones if not provided
            ...
            [row,col,vv] = find(adjacencyMatrix);
            ...
            op = 0.2;
            for i = 1:length(vv)
                if row(i) ~= col(i) && (this.LineMask(col(i)) == 0 && this.LineMask(row(i)) == 0) % plot mask on top
            ...
            for i = 1:length(vv)
                if row(i) ~= col(i) && (this.LineMask(col(i)) == 1 || this.LineMask(row(i)) == 1) % plot mask on top
                    if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0
                        % points are diametric, so draw a straight line
                        u = [cos(t(row(i)));sin(t(row(i)))];
                        v = [cos(t(col(i)));sin(t(col(i)))];
    ...
    methods (Static = true)
        ...
        function hideNodes(this,eventData)
            % Callback for 'Hide All' button
            n = this.UserData.Node;
            for i = 1:length(n)
                if eventData.Source.UserData.LineMask(i) == 0
                    n(i).Visible = false;
                end
            end
        end
